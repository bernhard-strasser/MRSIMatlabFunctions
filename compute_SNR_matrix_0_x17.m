function [SNR_mat,Shift_mat] = compute_SNR_matrix_0_x17(csi_mat_time,zerofilling,water_frequency,dwelltime,mask,out_dir,NoiseRegion_CS, ...
                               SeekNAA_PeakRegionWidth_IfWaterFound,SeekNAA_PeakRegionWidth_IfWaterNotFound,SeekNAA_InRealOrAbs,quiet,Debug_Mode)
%
...
SeekNAA_BasisRegionPeakdistance_HiFi_CS = 0.16;       % Compute the "Basis"-value in the HiFi region compared to peak at a Chemical Shift distance of this value (((0.08)))
SeekNAA_BasisRegionPeakdistance_LoFi_CS = 0.16;     % Same for LoFi
...
NoiseRegion_CS = sort(NoiseRegion_CS,'descend');
NoiseRegion_SP(1) = find(min(abs(CS_vec - NoiseRegion_CS(1))) == abs(CS_vec - NoiseRegion_CS(1)));
NoiseRegion_SP(2) = find(min(abs(CS_vec - NoiseRegion_CS(2))) == abs(CS_vec - NoiseRegion_CS(2)));
NoiseRegion_SP_vec = NoiseRegion_SP(1) : NoiseRegion_SP(2);
	...
	for y = 1:COL
        for x = 1:ROW
            ...
            if(~strcmp(quiet,'quiet'))
                display([ char(10) 'Processing Voxel z = ' num2str(z) ', y = ' num2str(y) ', x = ' num2str(x) ', Voxel ' num2str(CurrentVoxel) ' of ' num2str(TotalVoxels) ' . . .'])  
            end
            ...
            [Noise,Signal_Baseline,Signal_SubBas,Noise_ExcludeSubregions_StartEndPoints] = noise_computation_new_0_5(spectrum,[NoiseRegion_SP(1),NoiseRegion_SP(2)],'Apodize');
			...
			while(NoWaterPeak_found && (SeekWater_Step <= SeekWater_TotalSteps-1))
                ...
                if(strcmpi(SeekWater_InRealOrAbs,'real'))                                                                          
                    [NoWaterPeak_found,FoundWaterPeak_SP,FoundWaterPeak_SS,FoundWater_Basis_SP,FoundWater_Basis_SS,FoundWater_MeanBasis_SS,SeekWater_HiFiBasisRegion_SP,SeekWater_LoFiBasisRegion_SP] = ...
                     seek_peak_0_6(spectrum_zf,SeekWater_PeakRegion_SP,'real',0,'SNR_Criterion', ...
                    ...
                    [NoWaterPeak_found,FoundWaterPeak_SP,FoundWaterPeak_SS,FoundWater_Basis_SP,FoundWater_Basis_SS,FoundWater_MeanBasis_SS,SeekWater_HiFiBasisRegion_SP,SeekWater_LoFiBasisRegion_SP] = ...
                     seek_peak_0_6(spectrum_zf,SeekWater_PeakRegion_SP,'abs',0,'SNR_Criterion', ...
                     {{Noise,SeekWater_SNRthreshold,SeekWater_BasisRegionWidth_SP,SeekWater_BasisRegionPeakdistance_HiFi_SP,SeekWater_BasisRegionPeakdistance_LoFi_SP}});
                end
...
%              spectrum_zf_real,SeekNAA_PeakRegion_SP,Noise,'SNR_Criterion',{[SeekNAA_SNRthreshold,SeekNAA_BasisRegionWidth_SP,SeekNAA_BasisRegionPeakdistance_HiFi_SP, SeekNAA_BasisRegionPeakdistance_LoFi_SP]});
            ...
            [NoNAApeak_found,FoundNAAPeak_SP,FoundNAAPeak_SS,FoundNAA_Basis_SP,FoundNAA_Basis_SS,FoundNAA_MeanBasis_SS,SeekNAA_HiFiBasisRegion_SP,SeekNAA_LoFiBasisRegion_SP,SeekNAA_Polyfit,SeekNAA_SubBas] = seek_peak_0_7( ...
             spectrum_zf,SeekNAA_PeakRegion_SP,SeekNAA_InRealOrAbs,1,'SNR_Criterion',{{Noise,SeekNAA_SNRthreshold,SeekNAA_BasisRegionWidth_SP,SeekNAA_BasisRegionPeakdistance_HiFi_SP, SeekNAA_BasisRegionPeakdistance_LoFi_SP}});
...
%             end           
            ...
            if(exist('out_dir','var'))
                NoiseBase_fig = figure('visible','off');
                NoiseSubBase_fig = figure('visible','off');                
...
%               whole_spectrum_zf_fig = figure('visible','off');
                ...
                NoiseBase_fig = figure;
                NoiseSubBase_fig = figure;                
...
%               whole_spectrum_zf_fig = figure;  
            ...
            %% 9.1 Noise & Base
            set(0,'CurrentFigure',NoiseBase_fig)                                                          % set noise_figure as current figure to save data there
            plot(Plot_NoiseRegion_x,spectrum_real(NoiseRegion_SP_vec))
            hold on
            %plot(Plot_NoiseRegion_x,polyval(Polyfit_Noise_Real,NoiseRegion_SP_vec),'r')
            plot(Plot_NoiseRegion_x,real(Signal_Baseline(NoiseRegion_SP_vec)),'r')   
            hold off
            set(gca,'XDir','reverse');
            title(sprintf('Noise and Baseline, voxel x_%d y_%d z_%d', x,y,z),'Interpreter','none')
            xlabel('Chemical Shift')
            ylabel('Signal')            


             %% 9.2 NoiseSubBas
            set(0,'CurrentFigure',NoiseSubBase_fig)                                                          % set noise_figure as current figure to save data there
            plot(Plot_NoiseRegion_x,real(Signal_SubBas(NoiseRegion_SP_vec)))
            hold on
            % plot X-es in Regions that are excluded
            for ExcludeSubregions_LoopIndex = 1:size(Noise_ExcludeSubregions_StartEndPoints,2)
                StartPoint = Noise_ExcludeSubregions_StartEndPoints{ExcludeSubregions_LoopIndex}(1);
                EndPoint = Noise_ExcludeSubregions_StartEndPoints{ExcludeSubregions_LoopIndex}(2);
                StartCS = CS_vec(StartPoint);
                EndCS = CS_vec(EndPoint);
                plot([StartCS,EndCS],[min(real(Signal_SubBas(StartPoint:EndPoint))),max(real(Signal_SubBas(StartPoint:EndPoint)))],'g','Linewidth',1.8) % plot line increasing diagonal from left to right
                plot([StartCS,EndCS],[max(real(Signal_SubBas(StartPoint:EndPoint))),min(real(Signal_SubBas(StartPoint:EndPoint)))],'g','Linewidth',1.8) % plot line decreasing diagonal from left to right
            end  
            hold off
            set(gca,'XDir','reverse');
            title(sprintf('Noise Baseline subtracted, voxel x_%d y_%d z_%d', x,y,z),'Interpreter','none')
            xlabel('Chemical Shift')
            ylabel('Signal')            
...
%             
            ...
            if(exist('out_dir','var'))
                ...
                % 10.1 Noise Region & Baseline
                saveas(NoiseBase_fig,sprintf('%s/x_%s_y_%s_z_%s_NoiseAndBaseline', out_dir_FailedOrWon,num2str(x),num2str(y),num2str(z)),'fig')
                saveas(NoiseBase_fig,sprintf('%s/x_%s_y_%s_z_%s_NoiseAndBaseline', out_dir_FailedOrWon,num2str(x),num2str(y),num2str(z)),'jpg')
                
                % 10.2 Noise Region & Baseline
                saveas(NoiseSubBase_fig,sprintf('%s/x_%s_y_%s_z_%s_NoiseSubBas', out_dir_FailedOrWon,num2str(x),num2str(y),num2str(z)),'fig')
                saveas(NoiseSubBase_fig,sprintf('%s/x_%s_y_%s_z_%s_NoiseSubBas', out_dir_FailedOrWon,num2str(x),num2str(y),num2str(z)),'jpg')                
...
%                 saveas(succeeded_spectrum_fig,sprintf('%s/x_%s_y_%s_z_%s_NAApeak_and_basis.jpg', out_dir_FailedOrWon,num2str(x),num2str(y),num2str(z)))
            ...
            close(NoiseBase_fig,NoiseSubBase_fig, Metabo_spectrum_fig, SeekWater_fig, SeekNAA_fig);
