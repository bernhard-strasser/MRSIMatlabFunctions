function Compare_CSI_to_Images_1_0(file_struct,Hamming,LCM_add_phase,AllProbandsComparison_outdir)
...
for read_index = 1:size(file_struct,2)  % CSI DATA = LARGE --> COMPUTATIONS FOR EACH FILE INDIVIDUALLY
   ...
   %% 0. DEFINITIONS, PREPARATIONS
    ...
    if(LCM_flag)
        ...
        for LCM_phasemap_index = 1:size(LCM_phasemap_files,2)
            if(~strcmpi(LCM_phasemap_files{LCM_phasemap_index}, 'NO'))
                ...
                LCM_phasemap(LCM_phasemap_index,:,:) = fread(LCM_phasemap_fid, [64,64], 'float');
    ...
    % 2.2 ImNormal
    Image_Normal = circshift(Image_Normal, [0 1 -1]);   % 0 1 -1
...
%% 4. Channel LOOP
    ...
    %% 6.3 Statistical computations
        ...
        if(out_dir_flag)
            ...
            % 6.4.1 magnitudes
            saveas(fig_CSI_mag,sprintf('%s/channel%02d_%s_CSI_mag.fig', our_dir,plot_channel,file_struct(read_index).name))
            saveas(fig_CSI_mag,sprintf('%s/channel%02d_%s_CSI_mag.jpg', our_dir,plot_channel,file_struct(read_index).name))
            saveas(fig_ImNormal_mag,sprintf('%s/channel%02d_%s_ImNormal_mag.fig', our_dir,plot_channel,file_struct(read_index).name))
            saveas(fig_ImNormal_mag,sprintf('%s/channel%02d_%s_ImNormal_mag.jpg', our_dir,plot_channel,file_struct(read_index).name)) 

            if(Flip_flag)
                saveas(fig_ImFlip_mag,sprintf('%s/channel%02d_%s_ImFlip_mag.fig', our_dir,plot_channel,file_struct(read_index).name))
                saveas(fig_ImFlip_mag,sprintf('%s/channel%02d_%s_ImFlip_mag.jpg', our_dir,plot_channel,file_struct(read_index).name)) 
            end
            ...
            % 6.4.2 phasemaps
            % CSI
            saveas(fig_CSI_pha,sprintf('%s/channel%02d_%s_CSI_pha.fig', our_dir,plot_channel,file_struct(read_index).name))
            saveas(fig_CSI_pha,sprintf('%s/channel%02d_%s_CSI_pha.jpg', our_dir,plot_channel,file_struct(read_index).name))  
            % ImNormal
            saveas(fig_ImNormal_pha,sprintf('%s/channel%02d_%s_ImNormal_pha.fig', our_dir,plot_channel,file_struct(read_index).name))
            saveas(fig_ImNormal_pha,sprintf('%s/channel%02d_%s_ImNormal_pha.jpg', our_dir,plot_channel,file_struct(read_index).name)) 

            if(Flip_flag)
                % ImFlip
                saveas(fig_ImFlip_pha,sprintf('%s/channel%02d_%s_ImFlip_pha.fig', our_dir,plot_channel,file_struct(read_index).name))
                saveas(fig_ImFlip_pha,sprintf('%s/channel%02d_%s_ImFlip_pha.jpg', our_dir,plot_channel,file_struct(read_index).name)) 
                % ImCorr
                saveas(fig_ImCorr_pha,sprintf('%s/channel%02d_%s_ImCorr_pha.fig', our_dir,plot_channel,file_struct(read_index).name))
                saveas(fig_ImCorr_pha,sprintf('%s/channel%02d_%s_ImCorr_pha.jpg', our_dir,plot_channel,file_struct(read_index).name)) 
            end

            if(LCM_flag)
                % LCM
                saveas(fig_LCM,sprintf('%s/channel%02d_%s_LCM_pha.fig', our_dir,plot_channel,file_struct(read_index).name))
                saveas(fig_LCM,sprintf('%s/channel%02d_%s_LCM_pha.jpg', our_dir,plot_channel,file_struct(read_index).name))
            end
            ...
            % ImNormal
            saveas(fig_Sub_CSI_ImNormal,sprintf('%s/channel%02d_%s_Sub_CSI_Normal.fig', our_dir,plot_channel,file_struct(read_index).name))
            saveas(fig_Sub_CSI_ImNormal,sprintf('%s/channel%02d_%s_Sub_CSI_Normal.jpg', our_dir,plot_channel,file_struct(read_index).name))

            if(Flip_flag)
                % ImFlip
                saveas(fig_Sub_CSI_ImFlip,sprintf('%s/channel%02d_%s_Sub_CSI_Flip.fig', our_dir,plot_channel,file_struct(read_index).name))
                saveas(fig_Sub_CSI_ImFlip,sprintf('%s/channel%02d_%s_Sub_CSI_Flip.jpg', our_dir,plot_channel,file_struct(read_index).name))  
                % ImCorr
                saveas(fig_Sub_CSI_ImCorr,sprintf('%s/channel%02d_%s_Sub_CSI_Corr.fig', our_dir,plot_channel,file_struct(read_index).name))
                saveas(fig_Sub_CSI_ImCorr,sprintf('%s/channel%02d_%s_Sub_CSI_Corr.jpg', our_dir,plot_channel,file_struct(read_index).name))                    
            end
            ...
            % CSI
            if(LCM_flag)
                saveas(fig_Sub_LCM_CSI,sprintf('%s/channel%02d_%s_Sub_LCM_CSI.fig', our_dir,plot_channel,file_struct(read_index).name))
                saveas(fig_Sub_LCM_CSI,sprintf('%s/channel%02d_%s_Sub_LCM_CSI.jpg', our_dir,plot_channel,file_struct(read_index).name))  
                % ImNormal
                saveas(fig_Sub_LCM_ImNormal,sprintf('%s/channel%02d_%s_Sub_LCM_ImNormal.fig', our_dir,plot_channel,file_struct(read_index).name))
                saveas(fig_Sub_LCM_ImNormal,sprintf('%s/channel%02d_%s_Sub_LCM_ImNormal.jpg', our_dir,plot_channel,file_struct(read_index).name))  

                % ImFlip & ImCorr
                if(Flip_flag) 
                    saveas(fig_Sub_LCM_ImFlip,sprintf('%s/channel%02d_%s_Sub_LCM_ImFlip.fig', our_dir,plot_channel,file_struct(read_index).name))
                    saveas(fig_Sub_LCM_ImFlip,sprintf('%s/channel%02d_%s_Sub_LCM_ImFlip.jpg', our_dir,plot_channel,file_struct(read_index).name))  
                    saveas(fig_Sub_LCM_ImCorr,sprintf('%s/channel%02d_%s_Sub_LCM_ImCorr.fig', our_dir,plot_channel,file_struct(read_index).name))
                    saveas(fig_Sub_LCM_ImCorr,sprintf('%s/channel%02d_%s_Sub_LCM_ImCorr.jpg', our_dir,plot_channel,file_struct(read_index).name))    
                end %Flip_flag
            end %LCM_flag


            % mask
            saveas(fig_mask,sprintf('%s/%s_mask.fig', our_dir,file_struct(read_index).name))
            saveas(fig_mask,sprintf('%s/%s_mask.jpg', our_dir,file_struct(read_index).name))         
            ...
            if(~strcmpi(file_struct(read_index).mask_position,'NOSTATISTICS'))
                ...
                phasevalidation_txt_fid = fopen(sprintf('%s/channel%02d_phasevalidation_%s.txt',our_dir,plot_channel,file_struct(read_index).name),'a');
                ...
                if(~exist(sprintf('%s/statistics_AllProbandsComparison.txt',AllProbandsComparison_outdir),'file'))
                    fprintf(phasevalidation_txt_fid,'Proband_Channel           \tmean_Sub_LCM-Image\tstdd_Sub_LCM-Image\tmean_Sub_CSI-Image\tstdd_Sub_CSI-Image\n');                    
                end


                % write difference of CSI to Image and LCM to Image to file
                if(Flip_flag)
                    fprintf(AllProbandsComparison_fid,'%20.20s_channel%02d\t%18.4f\t%18.4f\t%18.4f\t%18.4f', ...
                    file_struct(read_index).name,plot_channel,Sub_CSI_ImCorr_mean,Sub_CSI_ImCorr_std,Sub_LCM_ImCorr_mean,Sub_LCM_ImCorr_std);               
                else 
                    fprintf(AllProbandsComparison_fid,'%20.20s_channel%02d\t%18.4f\t%18.4f\t%18.4f\t%18.4f', ...
                    file_struct(read_index).name,plot_channel,Sub_CSI_ImNormal_mean,Sub_CSI_ImNormal_std,Sub_LCM_ImNormal_mean,Sub_LCM_ImNormal_std);
                end
