function Compare_CSI_to_Images_1_3(file_struct,Hamming,AllProbandsComparison_outdir)
...
for read_index = 1:size(file_struct,2)  % CSI DATA = LARGE --> COMPUTATIONS FOR EACH FILE INDIVIDUALLY
   ...
   %% 0. DEFINITIONS, PREPARATIONS
    ...
    if(Phantom_flag)
        SubPhase = deg2rad(0); %63.2898
    else
        SubPhase = deg2rad(0); %30.0035
    end   
                ...
                for x = 1:ROW
                    for y = 1:COL
                        for z = 1:SLC
                            ...
                            fprintf(AllProbandsComparison_fid,'(%d,%d,%d)\t\t%14.4f\t%14.4f\t%14.4f\t%14.4f\t%14.4f\t%14.4f\n', ...
                            x,y,z,Sub_LCM_ImNormal(plot_channel,x,y),Sub_LCM_ImFlip(plot_channel,x,y),Sub_LCM_ImCorr(plot_channel,x,y),Sub_CSI_ImNormal(plot_channel,x,y),Sub_CSI_ImFlip(plot_channel,x,y),Sub_CSI_ImCorr(plot_channel,x,y));  
