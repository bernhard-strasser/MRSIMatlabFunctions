function SNR_mat = compute_SNR_matrix_0_7(csi_mat_time,zerofilling,water_frequency,dwelltime,mask,out_dir,Debug_Mode)
%
...
SeekWater_chemshift_Downfield = 4.65-0.14;		 % initial chemshift region to search for water
SeekWater_chemshift_upfield = 4.65+0.14;
SeekWater_chemshift_stepsize = 0.04;
SeekWater_totalsteps = 1;
...
%                 whole_spectrum_zf_fig = figure;  
            ...
            % Compute the NAA region to plot
...
%             ylabel('Signal')         
            ...
            % 12.5 Largest region where NAA was sought in real value
            ...
            plot(CS_vec_zf(seekNAA_SP_upfield),(0:spectrum_zf_real(NAApeak_SP)/500:spectrum_zf_real(NAApeak_SP)),CS_vec_zf(seekNAA_SP_Downfield),(0:spectrum_zf_real(NAApeak_SP)/500:spectrum_zf_real(NAApeak_SP)),'r'); 
            % Upfield SeekNAABasis_UpfieldRegion  
            plot(CS_vec_zf(NAA_UpfieldBasisRegion(1)),(0:spectrum_zf_real(NAApeak_SP)/500:spectrum_zf_real(NAApeak_SP)), ...
                 CS_vec_zf(NAA_UpfieldBasisRegion(2)),(0:spectrum_zf_real(NAApeak_SP)/500:spectrum_zf_real(NAApeak_SP)),'g');          
            % Upfield SeekNAABasis_DownfieldRegion
            plot(CS_vec_zf(NAA_DownfieldBasisRegion(1)),(0:spectrum_zf_real(NAApeak_SP)/500:spectrum_zf_real(NAApeak_SP)), ...
                 CS_vec_zf(NAA_DownfieldBasisRegion(2)),(0:spectrum_zf_real(NAApeak_SP)/500:spectrum_zf_real(NAApeak_SP)),'b');  
            ...
            xlabel('Chemical Shift')
            ylabel('Signal')
            legend('Measured Spectrum','NAA Peak','Upfield Basis','Downfield Basis','SeekNAAPeak_region','SeekNAABasis_UpfieldRegion', ...
                   'SeekNAABasis_DownfieldRegion','Location','Best')                 
