function [NoPeakFound,Peak_Point,Peak_Signal,Basis_Point,Basis_Signal,Basis_Signal_Mean,LeftBasisRegion,RightBasisRegion] = ...
          seek_peak_0_4(Signal,SeekPeakRegion,seek_criteria,seek_criteria_values)
% 
...
% 0.2 Subtract baseline of real and imaginary spectra

% 0.2.1 Fit Absolute Value
Polyfit_Signal_Abs = polyfit(SeekPeakRegion(1):SeekPeakRegion(2),abs(Signal(SeekPeakRegion(1):SeekPeakRegion(2))),3);                       % Fit baseline
Signal_Abs_SubBas = abs(Signal(SeekPeakRegion(1):SeekPeakRegion(2))) - polyval(Polyfit_Signal_Abs,SeekPeakRegion(1):SeekPeakRegion(2));      % Subtract that baseline

Signal_SubBas = abs(Signal);
%Signal_SubBas(SeekPeakRegion(1):SeekPeakRegion(2)) = Signal_Abs_SubBas;
...
if(SNR_crit_flag)
    ...
    Peak_Point = find(abs(Signal_SubBas(SeekPeakRegion(1):SeekPeakRegion(2))) == max(abs(Signal_SubBas(SeekPeakRegion(1):SeekPeakRegion(2)))));
...
if(mean_crit_flag)
    ...
    Polyfit_Signal_RealAbs = polyfit(SeekPeakRegion(1):SeekPeakRegion(2),real(Signal(SeekPeakRegion(1):SeekPeakRegion(2))),1);
