function [NoPeakFound,Peak_Point,Peak_Signal,Basis_Point,Basis_Signal,Basis_Signal_Mean,LeftBasisRegion,RightBasisRegion,Polyfit_Signal,Signal_SubBas] = ...
          seek_peak_1_2(Signal,SeekPeakRegion,Use_RealAbs,PolyfitOrder,Phantom_flag,seek_criteria,seek_criteria_values)
% 
...
if(SNR_crit_flag)
    ...
    % If in variables were omitted.
    if(numel(seek_criteria_values{SNR_crit_index}) < 3)
        BasisRegion_Width = 0;
...
%     end       
    ...
    % Peak Basis Criterion
    if(LeftBasis_Signal < Peak_Signal || RightBasis_Signal < Peak_Signal)
        PeakBasis_crit = true;
    else
        PeakBasis_crit = false;
    end

    % Basis Criterion: The Basis point signals should be closer than half of the Peak signal to the upper basis-signal.
    if(abs(LeftBasis_Signal - RightBasis_Signal) < abs(Peak_Signal - max(LeftBasis_Signal,RightBasis_Signal))*9/5)
        Basis_crit = true;
    else
        Basis_crit = false;
    end
    ...
    % SNR_criterion to 0 if no peak found, 1 if found; If no peak found, set SNR to NaN
    ...
    NoPeakFound = ~(SNR_crit * PeakBasis_crit * Basis_crit);
