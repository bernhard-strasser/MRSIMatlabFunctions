function Statistics = Compare_CSI_to_Images_1_6(file_struct,Hamming,AllProbandsComparison_outdir)
...
for read_index = 1:size(file_struct,2)  % CSI DATA = LARGE --> COMPUTATIONS FOR EACH FILE INDIVIDUALLY
   ...
   %% 0. DEFINITIONS, PREPARATIONS
    ...
    if(~strcmpi(file_struct(read_index).mask,'NO'))
        mask_flag = 1;
    end
    ...
    % 0.3 DEFINITIONS
    asc = read_ascconv_1_5(file_struct(read_index).CSI);
    ROW = asc.ROW; COL = asc.COL; SLC = asc.SLC; vecSize = asc.vecSize; total_channel_no = asc.total_channel_no;
    ...
    % 1.1 CSI
    CSI = read_csi_1_4(file_struct(read_index).CSI);
    
    
    % 1.2 Image NORMAL
    Image_Normal = read_image_1_3(file_struct(read_index).Image,[ROW,COL],'ZeroFilling',0,file_struct(read_index).fredir_shift,0,ROW/2-1,'AP');


    % 1.3 Image FLIP, COMPUTE CORR
    if(Flip_flag)
        Image_Flip = read_image_1_3(file_struct(read_index).Flip,[ROW,COL],'ZeroFilling',1,file_struct(read_index).fredir_shift,0,ROW/2-1,'AP');
    end 
    ...
    % 1.5 mask
	if(mask_flag && ischar(file_struct(read_index).mask)) % Read in
        mask = read_RawFiles_1_1(file_struct(read_index).mask,'float32',ROW,COL,1);
        mask = logical(mask);
    else
        ...
        elseif(~ischar(file_struct(read_index).mask))
            mask_position = file_struct(read_index).mask;          
        end
    ...
    if(Phantom_flag)
        SubPhase = deg2rad(62.567474); %63.2898
    else
        SubPhase = deg2rad(0); %29.606495   30.0035
    end   
    
    % 2.2 ImNormal
    Image_Normal = circshift(Image_Normal, [0 1 0]);   % 0 1 0;    0 1 -1
    ...
    % 2.3 ImFlip & ImCorr
    if(Flip_flag)
        Image_Flip = circshift(Image_Flip, [0 0 -1]); % 0 0 -1
    ...
    % 2.1 CSI
    CSI = reshape(squeeze(CSI(:,:,:,1,1)), [total_channel_no ROW COL]);
    if(strcmpi(Hamming,'hamming'))
        CSI = Hamming_filtering_1_2(CSI,[2 3]);
    end    
    
    %CSI = conj(CSI);
    CSI_phase = rad2deg(angle(conj(CSI)));   %reshape because if just 1 channel --> this dimension gets squeezed also!
    CSI_mag = abs(CSI);
...
%     end
    ...
    Sub_CSI_ImNormal = rad2deg(angle(CSI .* Image_Normal)); 
    %Sub_CSI_ImNormal = wrapTo180(Sub_CSI_ImNormal);
    
    if(Flip_flag)
        Sub_CSI_ImFlip = rad2deg(angle(CSI .* Image_Flip));
        Sub_CSI_ImCorr = rad2deg(angle(CSI .* Image_Corr));       
    ...
    if(LCM_flag)
        if(Phantom_flag)
            SubPhaseFromCSI = 40.005231273973322;
        else
            SubPhaseFromCSI = -23.230794077840983; % 15.208848923326459
        end
        
        
        
        Sub_LCM_CSI = wrapTo180(LCM_phasemap - (CSI_phase - SubPhaseFromCSI));
...
%         % 6.2.1 CSI AS REFERENCE
    ...
    %% 6.3 Statistical computations
        ...
        if(~strcmpi(file_struct(read_index).mask,'NOSTATISTICS'))
            ...
            Sub_CSI_ImNormal_mean = nanmean(Sub_CSI_ImNormal(plot_channel,mask))
            Sub_CSI_ImNormal_std = nanstd(Sub_CSI_ImNormal(plot_channel,mask))
            Sub_CSI_ImNormal_rss = sqrt(sum(Sub_CSI_ImNormal(plot_channel,mask).^2))


            if(Flip_flag)
                Sub_CSI_ImFlip_mean = nanmean(Sub_CSI_ImFlip(plot_channel,mask))
                Sub_CSI_ImFlip_std = nanstd(Sub_CSI_ImFlip(plot_channel,mask))
                Sub_CSI_ImFlip_rss = sqrt(sum(Sub_CSI_ImFlip(plot_channel,mask).^2))              
                Sub_CSI_ImCorr_mean = nanmean(Sub_CSI_ImCorr(plot_channel,mask))
                Sub_CSI_ImCorr_std = nanstd(Sub_CSI_ImCorr(plot_channel,mask))
                Sub_CSI_ImCorr_rss = sqrt(sum(Sub_CSI_ImCorr(plot_channel,mask).^2))                            
            end


            if(LCM_flag)
                Sub_LCM_ImNormal_mean = nanmean(Sub_LCM_ImNormal(plot_channel,mask))
                Sub_LCM_ImNormal_std = nanstd(Sub_LCM_ImNormal(plot_channel,mask))
                Sub_LCM_CSI_mean = nanmean(Sub_LCM_CSI(plot_channel,mask))
                Sub_LCM_CSI_std = nanstd(Sub_LCM_CSI(plot_channel,mask))

                if(Flip_flag)
                   Sub_LCM_ImFlip_mean = nanmean(Sub_LCM_ImFlip(plot_channel,mask))
                   Sub_LCM_ImFlip_std = nanstd(Sub_LCM_ImCorr(plot_channel,mask))
                   Sub_LCM_ImCorr_mean = nanmean(Sub_LCM_ImCorr(plot_channel,mask))
                   Sub_LCM_ImCorr_std = nanstd(Sub_LCM_ImCorr(plot_channel,mask))                 
                end %Flip_flag
            end % LCM_Flag
        end %if(NOSTATISTICS)
...
%                 fprintf(phasevalidation_txt_fid,'Sub_LCM_ImCorr    %9.4f  %9.4f\n\n',Sub_LCM_ImCorr_mean,Sub_LCM_ImCorr_std);
                ...
                % LCM file
                if(~exist(sprintf('%s/statistics_AllProbandsComparison.txt',AllProbandsComparison_outdir),'file'))
                    AllProbandsComparison_fid = fopen(sprintf('%s/statistics_AllProbandsComparison.txt',AllProbandsComparison_outdir),'w');
                    fprintf(AllProbandsComparison_fid,'%s',file_struct(read_index).name);
%                     NameWritten_flag = true;
%                     fprintf(AllProbandsComparison_fid,'\nchannel %d\n',plot_channel);
%                     ChannelWritten = plot_channel;
%                     fprintf(AllProbandsComparison_fid,'Voxel (ROW,COL,SLC)    \tSub_LCM-ImNor\tSub_LCM-ImFli\tSub_LCM-ImCor\tSub_CSI-ImNor\tSub_CSI_ImFli\tSub_CSI_ImCor\n');        
                else
%                     AllProbandsComparison_fid = fopen(sprintf('%s/statistics_AllProbandsComparison.txt',AllProbandsComparison_outdir),'a');
%                     if(exist('NameWritten_flag','var') && ~NameWritten_flag)
%                         fprintf(AllProbandsComparison_fid,'\n\n\n%s',file_struct(read_index).name);
%                         NameWritten_flag = true;
%                     end
%                     if(~exist('ChannelWritten','var') || ne(ChannelWritten,plot_channel))
%                     	fprintf(AllProbandsComparison_fid,'\nchannel %d\n',plot_channel);
%                         ChannelWritten = plot_channel;
%                     end
%                     if(exist('NameWritten_flag','var') && ~NameWritten_flag)                   
...
%                     end
                    ...
                    AllProbandsComparison_fid = fopen(sprintf('%s/statistics_AllProbandsComparison.txt',AllProbandsComparison_outdir),'a');
                        ...
                        %NameWritten_flag = true;
                       	...
                       	fprintf(AllProbandsComparison_fid,'\nchannel %d\n',plot_channel);
                        %ChannelWritten = plot_channel;
                     	fprintf(AllProbandsComparison_fid,'Voxel (ROW,COL,SLC)\tSub_LCM-ImNor\tSub_LCM-ImFli\tSub_LCM-ImCor\tSub_CSI-ImNor\tSub_CSI_ImFli\tSub_CSI_ImCor\tSub_LCM_CSI\n');                  
                
                
                eval([ 'Statistics.' file_struct(read_index).name '.voxels = cell(numel(mask(mask==1)),1);' ]);
                Statistics.SubMapNames = {'Sub_LCM_ImNormal','Sub_LCM_ImFlip' 'Sub_LCM_ImCorr' 'Sub_CSI_ImNormal' 'Sub_CSI_ImFlip' 'Sub_CSI_ImCorr','Sub_LCM_CSI'}; 
                eval([ 'Statistics.' file_struct(read_index).name '.data = zeros([numel(mask(mask==1)), 7]);' ]);
                ...
                for x = 1:ROW
                    for y = 1:COL
                        for z = 1:SLC
                            ...
                            voxelno = voxelno + 1;
                            eval([ 'Statistics.' file_struct(read_index).name '.voxels(voxelno) = {sprintf(''x%d_y%d_z%d'',x,y,z)};' ]);                           
                            
                            %x,y,z
                            %size(Sub_LCM_ImNormal), size(Sub_LCM_ImFlip), size(Sub_LCM_ImCorr(x,y)), size(Sub_CSI_ImNormal(x,y)), size(Sub_CSI_ImFlip(x,y)),size(Sub_CSI_ImCorr(x,y))
                            % write difference of CSI - Image and LCM - Image to file
                            if(Flip_flag)
                                fprintf(AllProbandsComparison_fid,'(%d,%d,%d)\t\t%14.4f\t%14.4f\t%14.4f\t%14.4f\t%14.4f\t%14.4f\t%14.4f\n', ...
                                x,y,z,Sub_LCM_ImNormal(plot_channel,x,y),Sub_LCM_ImFlip(plot_channel,x,y),Sub_LCM_ImCorr(plot_channel,x,y),Sub_CSI_ImNormal(plot_channel,x,y),Sub_CSI_ImFlip(plot_channel,x,y),Sub_CSI_ImCorr(plot_channel,x,y),Sub_LCM_CSI(plot_channel,x,y));
                                eval([ 'Statistics.' file_struct(read_index).name '.data(voxelno,:) = horzcat(Sub_LCM_ImNormal(plot_channel,x,y), Sub_LCM_ImFlip(plot_channel,x,y),Sub_LCM_ImCorr(plot_channel,x,y),Sub_CSI_ImNormal(plot_channel,x,y),Sub_CSI_ImFlip(plot_channel,x,y),Sub_CSI_ImCorr(plot_channel,x,y),Sub_LCM_CSI(plot_channel,x,y));' ]);                              
                            
                            else
                                fprintf(AllProbandsComparison_fid,'(%d,%d,%d)\t\t%14.4f\t%14.4f\t%14.4f\t%14.4f\t%14.4f\t%14.4f\t%14.4f\n', ...
                                x,y,z,Sub_LCM_ImNormal(plot_channel,x,y),Sub_LCM_ImNormal(plot_channel,x,y),Sub_LCM_ImNormal(plot_channel,x,y),Sub_CSI_ImNormal(plot_channel,x,y),Sub_CSI_ImNormal(plot_channel,x,y),Sub_CSI_ImNormal(plot_channel,x,y),Sub_LCM_CSI(plot_channel,x,y));
                                eval([ 'Statistics.' file_struct(read_index).name '.data(voxelno,:) = horzcat(Sub_LCM_ImNormal(plot_channel,x,y), Sub_LCM_ImNormal(plot_channel,x,y),Sub_LCM_ImNormal(plot_channel,x,y),Sub_CSI_ImNormal(plot_channel,x,y),Sub_CSI_ImNormal(plot_channel,x,y),Sub_CSI_ImNormal(plot_channel,x,y),Sub_LCM_CSI(plot_channel,x,y));' ]);                         
...
%                 file_struct(read_index).name,plot_channel,Sub_CSI_ImNormal_mean,Sub_CSI_ImNormal_std,Sub_CSI_ImFlip_mean,Sub_CSI_ImFlip_std,Sub_CSI_ImCorr_mean,Sub_CSI_ImCorr_std);               
                ...
                size(Sub_LCM_ImNormal)
                %eval([ 'Statistics.' file_struct(read_index).name '= Sub_LCM_ImNormal;' ])
                
                
                
                %fclose(phasevalidation_txt_fid);
                fclose(AllProbandsComparison_fid);
