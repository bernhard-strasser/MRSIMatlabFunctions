function [image_flag,image_flip_flag,image_weighting_flag,hamming_flag,phase_encod_dir_is,subbas_flag,dwelltime,water_frequency,pat_name,thickness,FOV_X,FOV_Y,freq_enc_dir_shift,freq_enc_dir_step, ...
         csi_file,basis_file,out_dir,image_file,image_flip_file,image_weighting_file,hamming_factor,sddegz,sddegp] = read_Parameter_txt_1_0_sctest(Parameter_txt)
...
dummy=fscanf(fid_Parameter,'%s',1);             compute_only_channel_flag=fscanf(fid_Parameter,'%d',1);
...
dummy = fscanf(fid_Parameter,'%s',4);            hamming_factor = fscanf(fid_Parameter,'%s',1);
...
dummy = fscanf(fid_Parameter,'%s',1);            compute_only_channel = fscanf(fid_Parameter,'%s',1);
