function [csi,csi_kspace] = read_csi_dat_2_0_Benchmark_ChannelByChannel_ExtraAssign(csi_path, zerofill_to_nextpow2_flag, zerofilling_fact, Hadamard_flag, x_shift,y_shift)
%
...
if(size(csi_kspace,2) > 1 || size(csi_kspace,3) > 1)                    % In case that MRSI data, not Single Voxel Spectroscopy data is inputted.
    ...
    for channel_no = 1: size(csi,1)
        ...
        tic
        csi_channel = flipdim(csi_channel,2);  
        csi(channel_no,:,:,:,:,:) = csi_channel;                                               % THIS FLIPS LEFT AND RIGHT IN SPATIAL DOMAIN BECAUSE PHYSICIANS WANT TO SEE IMAGES FLIPPED
