function [NoPeakFound,Peak_Point,Peak_Signal,Basis_Point,Basis_Signal,Basis_Signal_Mean,LeftBasisRegion,RightBasisRegion] = ...
          seek_peak_0_5(Signal,SeekPeakRegion,Use_RealAbs,seek_criteria,seek_criteria_values)
% 
...
Polyfit_Signal = eval([ 'polyfit(SeekPeakRegion(1):SeekPeakRegion(2),' Use_RealAbs '(Signal(SeekPeakRegion(1):SeekPeakRegion(2))),2);' ]);                       % Fit baseline
Signal_SubBas = eval([ Use_RealAbs '(Signal(SeekPeakRegion(1):SeekPeakRegion(2))) - polyval(Polyfit_Signal,SeekPeakRegion(1):SeekPeakRegion(2));']);      % Subtract that baseline

Signal_SubBas = eval([ Use_RealAbs '(Signal);' ]);
Signal_SubBas(SeekPeakRegion(1):SeekPeakRegion(2)) = Signal_SubBas;
