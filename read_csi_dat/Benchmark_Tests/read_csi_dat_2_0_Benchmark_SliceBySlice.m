function [csi,csi_kspace] = read_csi_dat_2_0_Benchmark_SliceBySlice(csi_path, zerofill_to_nextpow2_flag, zerofilling_fact, Hadamard_flag, x_shift,y_shift)
%
...
if(size(csi_kspace,2) > 1 || size(csi_kspace,3) > 1)                    % In case that MRSI data, not Single Voxel Spectroscopy data is inputted.
    ...
    for SLC_no = 1: SLC
        ...
        tic
        csi_slice = ifftshift(ifftshift(csi(:,:,:,SLC_no,:,:),2),3);
        ...
        tic
        csi(:,:,:,SLC_no,:,:) = flipdim(csi_slice,2);                                               % THIS FLIPS LEFT AND RIGHT IN SPATIAL DOMAIN BECAUSE PHYSICIANS WANT TO SEE IMAGES FLIPPED
        if(SLC_no == SLC)
            clear csi_slice
        end
