function [csi,csi_kspace] = read_csi_dat_2_0_Benchmark_SliceBySlice_AllIndexing(csi_path, zerofill_to_nextpow2_flag, zerofilling_fact, Hadamard_flag, x_shift,y_shift)
%
...
if(size(csi_kspace,2) > 1 || size(csi_kspace,3) > 1)                    % In case that MRSI data, not Single Voxel Spectroscopy data is inputted.
    ...
    for SLC_no = 1: SLC
        ...
        tic
        csi(:,:,:,SLC_no,:,:) = ifftshift(ifftshift(csi(:,:,:,SLC_no,:,:),2),3);
        display([ 'ifftshift took     ... ' num2str(toc) ' seconds' ])
        toc_sum = toc_sum + toc;
        
        tic
        csi(:,:,:,SLC_no,:,:) = fft(fft(csi(:,:,:,SLC_no,:,:),[],2),[],3);
        display([ 'fft took     ... ' num2str(toc) ' seconds' ])       
        toc_sum = toc_sum + toc;        
        
        tic
        csi(:,:,:,SLC_no,:,:) = conj(csi(:,:,:,SLC_no,:,:));                                                    % the chem shift gets higher from right to left --> conj reverses that
        display([ 'conj took     ... ' num2str(toc) ' seconds' ])               
        toc_sum = toc_sum + toc;
         
        tic
        csi(:,:,:,SLC_no,:,:) = fftshift(fftshift(csi(:,:,:,SLC_no,:,:),2),3);
        display([ 'fftshift took     ... ' num2str(toc) ' seconds' ])                       
        toc_sum = toc_sum + toc;
        
        tic
        csi(:,:,:,SLC_no,:,:) = flipdim(csi(:,:,:,SLC_no,:,:),2);                                               % THIS FLIPS LEFT AND RIGHT IN SPATIAL DOMAIN BECAUSE PHYSICIANS WANT TO SEE IMAGES FLIPPED
