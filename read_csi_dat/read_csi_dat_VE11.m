function [kSpace, Info] = read_csi_dat_VE11(file, DesiredSize,ReadInDataSets)
%
...
ParList = read_ascconv_VE11_eh(file);
...
% Analyze mdh.
ParList = Analyze_mdh_VE11(file,1);
...
fseek(file_fid, ParList.General.headersize+ParList.General.LastMeasOffset,'bof'); 
...
while(~ACQEND_flag)
    ...
    % Read mdh
    chak_header(1:5) = fread(file_fid, 5, 'uint32'); fseek(file_fid, 5*4,'cof'); 
	...
	if(~(sum(strcmpi(ReadInDataSets,'All')) || sum(strcmpi(CurrentMeasSet,ReadInDataSets))) )
		fseek(file_fid,192-(10*4+8+50*2) + (32+chak_header(8)*2*4)*chak_header(9),'cof');      % chak_header(9) ... total_channel_no, chak_header(8) ... ADC samples
		continue;
    end
    
    % Skipt rest of header
    fseek(file_fid,192-(10*4+8+50*2) + 32,'cof');
	...
	for CurCha = 1:Info.(CurrentMeasSet).total_channel_no
		...
		Info.(CurrentMeasSet).mdhInfo(3,CurInfoPt.(CurrentMeasSet)) = chak_header(18) + 1 - kSpaceShift.(CurrentMeasSet)(2);	% ky % FOR SOME STRANGE REASON THE "SEGMENT" INDEX IS USED INSTEAD OF THE PHASE INDEX (PHASE WOULD BE chak_header(15)
		...
		Info.(CurrentMeasSet).mdhInfo(10,CurInfoPt.(CurrentMeasSet)) = chak_header(23) *2;										% samples before echo
...
for CurrentMeasSet2 = transpose(fields(kSpace))
    ...
    % Remove field if dataset was skipped from reading in
    if(~(sum(strcmpi(ReadInDataSets,'All')) || sum(strcmpi(CurrentMeasSet,ReadInDataSets))))
        kSpace = rmfield(kSpace,CurrentMeasSet);
        Info = rmfield(Info,CurrentMeasSet);        
        continue
    end
