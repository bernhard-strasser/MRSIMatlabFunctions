function [SNR_mat,Shift_mat] = compute_SNR_matrix_0_x15(csi_mat_time,zerofilling,water_frequency,dwelltime,mask,out_dir,quiet,Debug_Mode)
%
...
NoiseRegion_LoFi_CS = 6.0;   % LoFi region where to compute the standard deviation of the noise    6  6
NoiseRegion_HiFi_CS = 7.5;   % Same for HiFi                                                     7.5  10.6
...
% figure
% plot(CS_vec(NoiseRegion_SP),real(csi_mat_freq_apod(NoiseRegion_SP)))
% set(gca,'XDir','reverse');
% pause
...
[Noise,Signal_Baseline,Signal_SubBas] = noise_computation_new_0_4(spectrum_zf,[NoiseRegion_HiFi_SP,NoiseRegion_LoFi_SP],'Apodize');
...
%               whole_spectrum_zf_fig = figure;  
            ...
            % Noise
            Plot_NoiseRegion_x = CS_vec(NoiseRegion_SP);
            ...
            plot(Plot_NoiseRegion_x,spectrum_real(NoiseRegion_SP))
            hold on
            %plot(Plot_NoiseRegion_x,polyval(Polyfit_Noise_Real,NoiseRegion_SP),'r')
            plot(Plot_NoiseRegion_x,Signal_Baseline(NoiseRegion_SP),'r')            
            hold off
            set(gca,'XDir','reverse');
            ...
            plot(Plot_NoiseRegion_x,Signal_SubBas(NoiseRegion_SP))
            set(gca,'XDir','reverse');
