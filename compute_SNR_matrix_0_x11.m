function [SNR_mat,Shift_mat] = compute_SNR_matrix_0_x11(csi_mat_time,zerofilling,water_frequency,dwelltime,mask,out_dir,Debug_Mode)
%
...
SeekNAA_PeakRegionWidth_IfWaterFound = 0.4;            % Seek for NAA in the region (IdealNAA_CS - Chemshift of WaterPeak +- this value) If WaterPeak was found
...
SeekNAA_BasisRegionPeakdistance_HiFi_CS = 0.15;       % Compute the "Basis"-value in the HiFi region compared to peak at a Chemical Shift distance of this value
SeekNAA_BasisRegionPeakdistance_LoFi_CS = 0.15;     % Same for LoFi
...
NoiseRegion_LoFi_CS = 6;   % LoFi region where to compute the standard deviation of the noise
NoiseRegion_HiFi_CS = 7.5;   % Same for HiFi
...
% csi_mat_time_apod = transpose(squeeze(csi_mat_time(14,10,SLC,:))) .* exp(-(1:vecSize)*0.007);
...
%                   WaterPeak_SP = WaterPeak_SP + SeekWater_PeakRegion_HiFi_SP - 1;     % because to the function only the chemshift region of interest is given. If function gives: peak at SP=1 
            ...
            [NAApeak_found,SNR,FoundNAAPeak_SP,FoundNAA_HiFiBasis_SP,FoundNAA_LoFiBasis_SP,FoundNAA_HiFiBasis_SS,FoundNAA_LoFiBasis_SS,FoundNAA_Basis_SS, ...
...
%             % DEBUG MODE MODE END
            ...
            if(error1_no_NAApeak)
                display('Seek ''n destroy NAA   F A I L E D!')
            else
                display('Seek ''n destroy NAA   S U C C E E D E D!')
            end
...
%               whole_spectrum_zf_fig = figure;  
            ...
            Plot_SeekNAA_Spec_y = spectrum_zf_real(Plot_NAAregion_HiFi_SP:Plot_NAAregion_LoFi_SP);
            ...
            % PEAK REGION VERTICAL LINE
            Plot_SeekNAA_PeakRegion_VertLine_Max = max(spectrum_zf_real(SeekNAA_PeakRegion_SP(1)),spectrum_zf_real(SeekNAA_PeakRegion_SP(2)));
            ...
            % HiFi BASIS REGION VERTICAL LINE
            Plot_SeekNAA_HiFiBasisRegion_VertLine_Max = max(spectrum_zf_real(SeekNAA_HiFiBasisRegion_SP(1)),spectrum_zf_real(SeekNAA_HiFiBasisRegion_SP(2)));
            ...
            % LoFi BASIS REGION VERTICAL LINE
            Plot_SeekNAA_LoFiBasisRegion_VertLine_Max = max(spectrum_zf_real(SeekNAA_LoFiBasisRegion_SP(1)),spectrum_zf_real(SeekNAA_LoFiBasisRegion_SP(2)));
            ...
            hold on
            plot(Plot_NoiseRegion_x,polyval(Polyfit_Noise_Real,Plot_NoiseRegion_x),'r')
            hold off
            set(gca,'XDir','reverse');
...
%             ylabel('Signal')         
            ...
            plot(CS_vec_zf(FoundNAAPeak_SP),spectrum_zf_real(FoundNAAPeak_SP),'o','MarkerSize',6,'MarkerFaceColor','r','MarkerEdgeColor','k');                               % Where peak was computed
            plot(CS_vec_zf(FoundNAA_HiFiBasis_SP),FoundNAA_HiFiBasis_SS,'o','MarkerSize',6,'MarkerFaceColor','c','MarkerEdgeColor','k');                    % Where HiFi_Basis was computed 
            plot(CS_vec_zf(FoundNAA_LoFiBasis_SP),FoundNAA_LoFiBasis_SS,'o','MarkerSize',6,'MarkerFaceColor','b','MarkerEdgeColor','k');                         % Where LoFi_Basis was computed
            NAALegendHandi(1) = plot(CS_vec_zf(FoundNAAPeak_SP),FoundNAA_Basis_SS,'o','MarkerSize',6,'MarkerFaceColor','g','MarkerEdgeColor','k');
...
%             plot(CS_vec_zf(FoundNAA_HiFiBasis_SP:FoundNAA_LoFiBasis_SP),spectrum_zf_real(FoundNAA_HiFiBasis_SP:FoundNAA_LoFiBasis_SP),'k','Linewidth',1.6)
% %             hold on
% %             plot(CS_vec_zf(FoundNAAPeak_SP),spectrum_zf_real(FoundNAAPeak_SP),'o','MarkerSize',6,'MarkerFaceColor','r','MarkerEdgeColor','k')
% %             plot(CS_vec_zf(FoundNAA_HiFiBasis_SP),spectrum_zf_real(FoundNAA_HiFiBasis_SP),'o','MarkerSize',6,'MarkerFaceColor','g','MarkerEdgeColor','k')
% %             plot(CS_vec_zf(FoundNAA_LoFiBasis_SP),spectrum_zf_real(FoundNAA_LoFiBasis_SP),'o','MarkerSize',6,'MarkerFaceColor','b','MarkerEdgeColor','k')
