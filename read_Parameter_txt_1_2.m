function [image_flag,image_flip_flag,image_weighting_flag,hamming_flag,phase_encod_dir_is,subbas_flag,use_phantom_flag,dwelltime,water_frequency,pat_name,thickness,FOV_X,FOV_Y,ROW,COL,SLC,freq_enc_dir_shift, ...
          freq_enc_dir_step, csi_file,basis_file,out_dir,image_file,image_flip_file,image_weighting_file,hamming_factor,sddegz,sddegp] = read_Parameter_txt_1_2(Parameter_txt)
...
dummy=fscanf(fid_Parameter,'%s',1);             use_phantom_flag=fscanf(fid_Parameter,'%d',1);
...
dummy = fscanf(fid_Parameter,'%s',3);            ROW = fscanf(fid_Parameter,'%d',1); COL = fscanf(fid_Parameter,'%d',1); SLC = fscanf(fid_Parameter,'%d',1);
