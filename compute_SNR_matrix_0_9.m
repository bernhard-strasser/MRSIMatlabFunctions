function SNR_mat = compute_SNR_matrix_0_9(csi_mat_time,zerofilling,water_frequency,dwelltime,mask,out_dir,Debug_Mode)
%
...
%             pause
			...
			while(~WaterPeak_found && (SeekWater_Step <= SeekWater_TotalSteps-1))
                ...
                SeekWater_PeakRegion_SP = [SeekWater_PeakRegion_Upfield_SP, SeekWater_PeakRegion_Downfield_SP];

				% 6.1 Check for WaterPeak in real part: Criteria
                if(strcmpi(SeekWater_InRealOrAbs,'real'))                                                                          
                    [WaterPeak_found,SNR,WaterPeak_SP,Water_UpfieldBasis_SP,Water_DownfieldBasis_SP,Water_Basis_Signal,SeekWater_UpfieldBasisRegion_SP,SeekWater_DownfieldBasisRegion_SP] = ...
                    seek_peak_0_1(spectrum_zf_real,SeekWater_PeakRegion_SP,Noise, ...
                    ...
                    [WaterPeak_found,SNR,WaterPeak_SP,Water_UpfieldBasis_SP,Water_DownfieldBasis_SP,Water_Basis_Signal,SeekWater_UpfieldBasisRegion_SP,SeekWater_DownfieldBasisRegion_SP] = ...
                    seek_peak_0_1(spectrum_zf_abs,SeekWater_PeakRegion_SP,Noise, ...
                    {'SNR_Criterion'},{[SeekWater_SNRthreshold,SeekWater_BasisRegionWidth_SP,SeekWater_BasisRegionPeakdistance_Upfield_SP,SeekWater_BasisRegionPeakdistance_Downfield_SP]});
                end
...
%                   WaterPeak_SP = WaterPeak_SP + SeekWater_PeakRegion_Upfield_SP - 1;     % because to the function only the chemshift region of interest is given. If function gives: peak at SP=1 
            ...
            seekNAA_PeakRegion_SP = [SeekNAA_PeakRegion_Upfield_SP, SeekNAA_PeakRegion_Downfield_SP];
            ...
            [NAApeak_found,SNR,NAApeak_SP,NAA_UpfieldBasis_SP,NAA_DownfieldBasis_SP,NAA_Basis_Signal,NAA_UpfieldBasisRegion,NAA_DownfieldBasisRegion] = seek_peak_0_1( ...
             spectrum_zf_real,seekNAA_PeakRegion_SP,Noise,'SNR_Criterion',{[seekNAA_SNRthreshold,SeekNAA_BasisRegionWidth_SP,SeekNAA_BasisRegionPeakdistance_Upfield_SP, SeekNAA_BasisRegionPeakdistance_Downfield_SP]});
...
%               whole_spectrum_zf_fig = figure;  
            ...
            Plot_NAAregion_Upfield_SP = min(min(seekNAA_PeakRegion_SP), NAA_UpfieldBasisRegion);
            Plot_NAAregion_Downfield_SP = max(max(seekNAA_PeakRegion_SP), NAA_DownfieldBasisRegion);

            Plot_WaterRegion_Upfield_SP = min(min(SeekWater_PeakRegion_SP), SeekWater_UpfieldBasisRegion_SP);
            Plot_WaterRegion_Downfield_SP = max(max(SeekWater_PeakRegion_SP), SeekWater_DownfieldBasisRegion_SP);



            % 9.0.3 Define the vectors to plot
            
            % Noise
            Plot_NoiseRegion_x = CS_vec(NoiseRegion_Upfield_SP:NoiseRegion_Downfield_SP);
            
            % SeekWater
            % SPECTRUM ITSELF
            Plot_SeekWater_Spec_x = CS_vec_zf(Plot_WaterRegion_Upfield_SP:Plot_WaterRegion_Downfield_SP);
            Plot_SeekWater_Spec_y = eval([ 'spectrum_zf_' SeekWater_InRealOrAbs '(Plot_WaterRegion_Upfield_SP:Plot_WaterRegion_Downfield_SP)' ]);       % plot either real or absolute value depending on where water was sought
            
            % PEAK REGION VERTICAL LINE
            Plot_SeekWater_MaxPeakRegion = max(spectrum_zf_real(SeekWater_PeakRegion_SP(1)),spectrum_zf_real(SeekWater_PeakRegion_SP(2))); 
            Plot_SeekWater_PeakRegion_y = min(Plot_SeekWater_Spec_y):(Plot_SeekWater_MaxPeakRegion-min(Plot_SeekWater_Spec_y))/1:Plot_SeekWater_MaxPeakRegion;
            Plot_SeekWater_PeakRegion_Upfield_x = repmat(CS_vec_zf(SeekWater_PeakRegion_SP(1)),[1 2]);                % if you use just plot(x,vector), x scalar, only dots get plotted
            Plot_SeekWater_PeakRegion_Downfield_x = repmat(CS_vec_zf(SeekWater_PeakRegion_SP(2)),[1 2]);              % So plot([x,x],[y_start,y_end]) 
            
            % UPFIELD BASIS REGION VERTICAL LINE
            Plot_SeekWater_MaxUpfieldBasisRegion = max(spectrum_zf_real(SeekWater_UpfieldBasisRegion_SP(1)),spectrum_zf_real(SeekWater_UpfieldBasisRegion_SP(2)));
            Plot_SeekWater_UpfieldBasisRegion_y = min(Plot_SeekWater_Spec_y):(Plot_SeekWater_MaxUpfieldBasisRegion-min(Plot_SeekWater_Spec_y))/1:Plot_SeekWater_MaxUpfieldBasisRegion;
            Plot_SeekWater_UpfieldBasisRegion_Upfield_x = repmat(CS_vec_zf(SeekWater_UpfieldBasisRegion_SP(1)),[1 2]);
            Plot_SeekWater_UpfieldBasisRegion_Downfield_x = repmat(CS_vec_zf(SeekWater_UpfieldBasisRegion_SP(2)), [1 2]);
            
            % DOWNFIELD BASIS REGION VERTICAL LINE
            Plot_SeekWater_MaxDownfieldBasisRegion = max(spectrum_zf_real(SeekWater_DownfieldBasisRegion_SP(1)),spectrum_zf_real(SeekWater_DownfieldBasisRegion_SP(2)));
            Plot_SeekWater_DownfieldBasisRegion_y = min(Plot_SeekWater_Spec_y):(Plot_SeekWater_MaxDownfieldBasisRegion-min(Plot_SeekWater_Spec_y))/1:Plot_SeekWater_MaxDownfieldBasisRegion;
            Plot_SeekWater_DownfieldBasisRegion_Upfield_x = repmat(CS_vec_zf(SeekWater_DownfieldBasisRegion_SP(1)), [1 2]);
            Plot_SeekWater_DownfieldBasisRegion_Downfield_x = repmat(CS_vec_zf(SeekWater_DownfieldBasisRegion_SP(2)), [1 2]);            
            
            

            % 9.1 Noise Region
            set(0,'CurrentFigure',noise_fig)                                                          % set noise_figure as current figure to save data there
            plot(Plot_NoiseRegion_x,Noise_Spectrum_Real)
            set(gca,'XDir','reverse');
...
%             ylabel('Signal')         
            ...
            plot(Plot_SeekWater_PeakRegion_Upfield_x,Plot_SeekWater_PeakRegion_y, '--r',Plot_SeekWater_PeakRegion_Downfield_x,Plot_SeekWater_PeakRegion_y,'--r','Linewidth',1.6)
            % Upfield SeekWaterBasis_UpfieldRegion 
            plot(Plot_SeekWater_UpfieldBasisRegion_Upfield_x,Plot_SeekWater_UpfieldBasisRegion_y,'--g',Plot_SeekWater_UpfieldBasisRegion_Downfield_x,Plot_SeekWater_UpfieldBasisRegion_y,'--g','Linewidth',1.6)        
            % Upfield SeekWaterBasis_DownfieldRegion
            plot(Plot_SeekWater_DownfieldBasisRegion_Upfield_x,Plot_SeekWater_DownfieldBasisRegion_y,'--b',Plot_SeekWater_DownfieldBasisRegion_Downfield_x,Plot_SeekWater_DownfieldBasisRegion_y,'--b','Linewidth',1.6); 
            ...
            xlabel('Chemical Shift')
            ylabel('Signal')
            leg = legend('Measured Spectrum','Peak','Upfield Basis','Downfield Basis','Peak Region','Upfield Basis Region','Downfield Basis Region','Location','Best');
            set(leg,'FontSize',6);
            ...
            plot(CS_vec_zf(SeekNAA_PeakRegion_Upfield_SP),(0:spectrum_zf_real(NAApeak_SP)/50:spectrum_zf_real(NAApeak_SP)),'r', ...
            CS_vec_zf(SeekNAA_PeakRegion_Downfield_SP),(0:spectrum_zf_real(NAApeak_SP)/50:spectrum_zf_real(NAApeak_SP)),'--r','Linewidth',2.6); 
            % Upfield SeekNAABasis_UpfieldRegion  
            plot(CS_vec_zf(NAA_UpfieldBasisRegion(1)),(0:spectrum_zf_real(NAApeak_SP)/50:spectrum_zf_real(NAApeak_SP)), 'g', ...
                 CS_vec_zf(NAA_UpfieldBasisRegion(2)),(0:spectrum_zf_real(NAApeak_SP)/50:spectrum_zf_real(NAApeak_SP)),'g','Linewidth',2.6);          
            % Upfield SeekNAABasis_DownfieldRegion
            plot(CS_vec_zf(NAA_DownfieldBasisRegion(1)),(0:spectrum_zf_real(NAApeak_SP)/50:spectrum_zf_real(NAApeak_SP)), 'b', ...
                 CS_vec_zf(NAA_DownfieldBasisRegion(2)),(0:spectrum_zf_real(NAApeak_SP)/50:spectrum_zf_real(NAApeak_SP)),'b','Linewidth',2.6);  
