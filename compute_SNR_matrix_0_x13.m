function [SNR_mat,Shift_mat] = compute_SNR_matrix_0_x13(csi_mat_time,zerofilling,water_frequency,dwelltime,mask,out_dir,quiet,Debug_Mode)
%
...
NoiseRegion_LoFi_CS = 6.0;   % LoFi region where to compute the standard deviation of the noise    6
NoiseRegion_HiFi_CS = 10.6;   % Same for HiFi                                                     7.5
...
%              spectrum_zf_real,SeekNAA_PeakRegion_SP,Noise,'SNR_Criterion',{[SeekNAA_SNRthreshold,SeekNAA_BasisRegionWidth_SP,SeekNAA_BasisRegionPeakdistance_HiFi_SP, SeekNAA_BasisRegionPeakdistance_LoFi_SP]});
            ...
            [NoNAApeak_found,FoundNAAPeak_SP,FoundNAAPeak_SS,FoundNAA_Basis_SP,FoundNAA_Basis_SS,FoundNAA_MeanBasis_SS,SeekNAA_HiFiBasisRegion_SP,SeekNAA_LoFiBasisRegion_SP,Polyfit_Signal_Real,Polyfit_Signal_Imag] = seek_peak_0_3( ...
...
%               whole_spectrum_zf_fig = figure;  
            ...
            % Baseline
            Plot_SeekNAA_Baseline = polyval(Polyfit_Signal_Real,Plot_NAAregion_HiFi_SP:Plot_NAAregion_LoFi_SP) + 1i*polyval(Polyfit_Signal_Imag,Plot_NAAregion_HiFi_SP:Plot_NAAregion_LoFi_SP);
            Plot_SeekNAA_Baseline = eval([ SeekNAA_InRealOrAbs '(Plot_SeekNAA_Baseline);']);
...
%             ylabel('Signal')         
            ...
            % Baseline
            NAALegendHandi(5) = plot(Plot_SeekNAA_Spec_x,Plot_SeekNAA_Baseline);
            ...
            xlabel('Chemical Shift')
            ylabel('Signal')
            leg = legend(NAALegendHandi, 'Mean Basis','Peak Region','HiFi Basis Region','LoFi Basis Region','Baseline');
            set(leg,'FontSize',6);
            set(leg,'Location','Best');
