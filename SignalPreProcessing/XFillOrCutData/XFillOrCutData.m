function OutArray = XFillOrCutData(OutArray,Zerofill_To,PerformFFT_flag,X)
%
...
for dummy_dim = ApplyAlongDims
	...
	if(AppendZeros(dummy_dim) > 0)
		AppendZeros_AtBeginning = myrepmat(X,[size_OutArray(1:dummy_dim-1) ceil(AppendZeros(dummy_dim)/2) size_OutArray(dummy_dim+1:end)]);
		AppendZeros_AtEnd = myrepmat(X,[size_OutArray(1:dummy_dim-1) floor(AppendZeros(dummy_dim)/2) size_OutArray(dummy_dim+1:end)]);
