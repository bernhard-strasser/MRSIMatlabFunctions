function [OutArray,mask] = EllipticalFilter_5D(InArray,ApplyAlongDims,EllipsoidCoefficients,InputIskSpace_flag)
%
...
if(numel(ApplyAlongDims) == 2)
    ...
    [x_grid,y_grid,z_grid, v_grid, w_grid] = ndgrid(1:size(OutArray,ApplyAlongDims(1)), 1:size(OutArray,ApplyAlongDims(2)), 1:size(OutArray,ApplyAlongDims(3)), 1:size(OutArray,ApplyAlongDims(4)), 1:size(OutArray,ApplyAlongDims(5)));   
end

mask = ( (x_grid - kSpaceCenter(1)) / EllipsoidCoefficients(1) ).^2 + ( (y_grid - kSpaceCenter(2)) / EllipsoidCoefficients(2) ).^2 + ( (z_grid - kSpaceCenter(3)) / EllipsoidCoefficients(3) ).^2 + ( (v_grid - kSpaceCenter(4)) / EllipsoidCoefficients(3) ).^2 + ( (w_grid - kSpaceCenter(5)) / EllipsoidCoefficients(3) ).^2 <= EllipsoidCoefficients(4)^2+Epsilon;
